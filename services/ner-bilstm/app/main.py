...
async def startup_event():
    ...
    if DOCS_BUILD:
        logger.info("DOCS_BUILD=1; skipping BiLSTM model load")
        _model_loaded = False
        return

    logger.info("Starting BiLSTM NER service")

    try:
        logger.info(f"Loading BiLSTM model from: {settings.model_dir}")
...
async def shutdown_event():
    ...
    logger.info("Shutting down BiLSTM NER service")
        ...
        raise HTTPException(
            ...
            detail={
                "error": {
                    ...
                    "message": "BiLSTM model not initialized",
